...
import { validateTextQuality, TextQualityValidator } from '@/lib/utils/text-validation';
...
const TEST_SCENARIOS = {
  ...
  VALID_TEXT: {
    ...
    vietnamese_long: 'Công ty TNHH ABC xin thông báo về việc thay đổi địa chỉ trụ sở chính. Địa chỉ mới sẽ có hiệu lực từ ngày 01/02/2025. Mọi thông tin chi tiết xin liên hệ qua hotline 1900-xxxx.',
    english_short: 'This is a valid English document with meaningful content.',
    english_long: 'This document contains important information about the company policy changes. Please review carefully and contact HR department if you have any questions regarding the new procedures.',
    mixed_languages: 'Company ABC thông báo về new policy thay đổi từ ngày 01/02/2025. Contact HR for more information.',
    contract_sample: 'AGREEMENT\n\nThis Agreement is entered into between Party A and Party B for the purpose of establishing terms and conditions for the services to be provided.',
  },
  ...
  REAL_WORLD: {
    ...
    poor_ocr_output: 'Thi s i s a p o o r O C R o u t p u t w i t h s p a c e s',
    table_dots: 'Name: ................ Age: ................ Address: ................',
    form_fields: '1. _________________ 2. _________________ 3. _________________',
  }
};
...
function runValidationTest(
  ...
  console.log(`🎯 Expected: ${expectedValid ? 'VALID (no OCR)' : 'INVALID (trigger OCR)'}`);
  
  const result = validateTextQuality(text);
  
  console.log(`📊 Result: ${result.isValid ? 'VALID' : 'INVALID'} (confidence: ${result.confidence.toFixed(3)})`);
  console.log(`📝 Reason: ${result.reason}`);
  console.log(`📈 Metrics:`);
  console.log(`   - Characters: ${result.metrics.charLength}`);
  console.log(`   - Syllables: ${result.metrics.syllableCount}`);
  console.log(`   - Syllable Density: ${result.metrics.syllableDensity.toFixed(4)}`);
  ...
  if (!passed) {
    console.log(`💡 Expected ${expectedValid ? 'valid' : 'invalid'} but got ${result.isValid ? 'valid' : 'invalid'}`);
  }
  
  if (expectedReason && !result.reason.toLowerCase().includes(expectedReason.toLowerCase())) {
    console.log(`⚠️ Expected reason to contain "${expectedReason}" but got "${result.reason}"`);
  }
...
function runPerformanceTest(text: string, testName: string): number {
  ...
  console.log(`${avgTime < 50 ? '✅ GOOD' : avgTime < 100 ? '⚠️ ACCEPTABLE' : '❌ SLOW'} - Performance`);
...
export function runOcrV2TestSuite(): void {
  ...
  for (const [key, text] of Object.entries(TEST_SCENARIOS.PROBLEMATIC)) {
    const passed = runValidationTest(text, false, `Problematic: ${key}`, 'low');
    totalTests++;
  ...
  for (const [key, text] of Object.entries(TEST_SCENARIOS.VALID_TEXT)) {
    const passed = runValidationTest(text, true, `Valid: ${key}`);
    totalTests++;
    if (passed) passedTests++;
  }
  ...
  for (const [key, text] of Object.entries(TEST_SCENARIOS.EDGE_CASES)) {
    const expected = edgeCaseExpectations[key as keyof typeof edgeCaseExpectations];
    const passed = runValidationTest(text, expected, `Edge Case: ${key}`);
    totalTests++;
    if (passed) passedTests++;
  }
  ...
  for (const [key, text] of Object.entries(TEST_SCENARIOS.REAL_WORLD)) {
    const passed = runValidationTest(text, false, `Real-world: ${key}`, 'low');
    totalTests++;
    if (passed) passedTests++;
  }
  ...
  const performanceResults = [
    runPerformanceTest(TEST_SCENARIOS.VALID_TEXT.vietnamese_long, 'Vietnamese Long Text'),
    runPerformanceTest(TEST_SCENARIOS.PROBLEMATIC.dots_only, 'Dots Only'),
    runPerformanceTest(TEST_SCENARIOS.VALID_TEXT.english_long, 'English Long Text'),
  ];
  ...
  console.log(`   - Confidence Threshold: ${OCR_CONFIG.OCR_TRIGGER_CONFIDENCE_THRESHOLD}`);
  ...
  const result = validator.validateTextQuality(boundaryText);
  console.log(`🧪 Boundary Test (threshold 0.3): confidence=${result.confidence.toFixed(3)}, valid=${result.isValid}`);
  ...
  console.log(`Average Performance: ${avgPerformance.toFixed(2)}ms per validation`);
...
export async function testApiIntegration(): Promise<void> {
  ...
  for (const testCase of testCases) {
    ...
    try {
      ...
      const mockPdfBlob = new Blob([testCase.text], { type: 'application/pdf' });
...
export function runRegressionTests(): void {
  ...
  legacyResults.forEach(({ text, legacy }, index) => {
    const newResult = validateTextQuality(text);
    ...
    console.log(`   Improvement: ${!lengthCheck && !newResult.isValid ? '✅ Fixed' : lengthCheck && newResult.isValid ? '✅ Maintained' : '⚠️ Changed'}`);
  });
}
...
if (typeof window === 'undefined') {
  ...
  console.log('🌐 Test suite loaded. Call runOcrV2TestSuite() to execute tests.');
}